...
class TCNAE:
    ...
    def build_model(self, verbose=1):
        ...
        tcn_enc = TCN(nb_filters=self.nb_filters,
        ...
        enc_flat = Conv1D(filters=self.filters_conv1d,
                          kernel_size=1,
                          activation=self.activation_conv1d,
                          padding=self.padding)(tcn_enc)

        # Do some average (max) pooling to get a compressed representation of the time series (e.g. a sequence of
        # length 8)
        ...
        dec_reconstructed = TCN(nb_filters=self.nb_filters,
                                kernel_size=self.kernel_size,
                                nb_stacks=self.nb_stacks,
                                dilations=self.dilations,
                                padding=self.padding,
        ...
        adam = Adam(learning_rate=self.lr, beta_1=0.9, beta_2=0.999, epsilon=1e-08, decay=0.0, amsgrad=True)
        model.compile(loss=self.loss, optimizer=adam, metrics=[self.loss])
        if verbose > 1:
            model.summary()
        self.model = model

    def fit(self, train_X, train_Y, batch_size=32, epochs=40, verbose=1, save=False):
        my_callbacks = []
        if self.use_early_stopping:
            my_callbacks = [EarlyStopping(monitor='val_loss', patience=2, min_delta=1e-4, restore_best_weights=True)]
        if self.log_tensorboard:
            my_callbacks.extend([TensorBoard(log_dir="./logs")])
        keras_verbose = 0
        if verbose > 0:
    ...
    def load(self):
        self.model = load_model(f"models/{self.name}")
